...
VIP Game Manual より。
これと、次の VIP Roundup は Brian Astle さんの作。
VIP 用のプログラムには、入力の手間も考えたのか、
タイトル画面などが無いものが多く、
たとえば Game Manual の VIP Pinball など、
まずはブランク画面上でプレイヤー数を入力して
はじめてゲーム画面が現れる、という作りになっています。
それに対して Brian Astle さんのゲームは、
タイトル画面からエンディングまで作ってあり、
完成したアプリケーションという感じがします。

Brian Astle さんには VIPER vol. 1, issue 5 の LIEF on VIP もありますが、
これはカセットサービスとなっておりコードが載っていないので試せていません。
どこかにあるのでしょうか。

VIP Animal Race は、動物のレースにチップを賭けて増やしてゆく、というゲームです。
左が賭ける画面で、微妙にスタート位置が違い、それに対応してオッズが決まっています。
また、時々オッズが全体的に高いことがあり、その時が狙い目です。

![Animal Race Bet](animal_bet.png)
![Animal Race Race](animal_race.png)

256 ドルになったら勝ちで、エンディング画面が見られます。

![Animal Race Win](animal_win.png)



## VIP Round Up

VIP Game Manual II より。

5頭の犬を動かして羊を柵の中に入れてゆくのですが、
羊は全然思い通りに動きません。
行かせたくない方向に犬を配置して、
羊がたまたま柵の方に行くのを待つ、という気持ちでいると良さそうです。

Speed=3 で、なんとかクリアしたところ。
TIME が 186 となってますが、秒単位ではないと思います
(数十分かかった気がする。)


![Roundup Win](roundup_win.png)
